...
import { useSenderEntities } from "@/hooks/useSenderEntities";

export default function SenderEntitiesPage() {
  // ===== Auth Guard =====
  const { isLoading: isAuthLoading, isAuthorized } = useAuthGuard({
    requiredPermissions: [PERMISSIONS.MANAGE_SENDER_ENTITIES],
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await updateEntity(editing.id, name);
      } else {
  ...
  const confirmDelete = async () => {
    ...
    setProcessing(true);
    try {
      await deleteEntity(deleteTargetId);
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-slate-50 p-3 sm:p-4">
              ...
              <table className="w-full text-right text-xs sm:text-sm">
                <thead>
                  <tr className="bg-blue-50 text-slate-700">
                    ...
                    <th className="p-2 sm:p-3 font-semibold hidden md:table-cell">
                      تاريخ الإنشاء
                    </th>
                    <th className="p-2 sm:p-3 font-semibold hidden lg:table-cell">
                      آخر تحديث
                    </th>
      ...
      <ConfirmationModal
        ...
        title={deleteError ? "فشل الحذف" : "حذف الجهة المرسلة"}
        message={
          deleteError 
            ? `⚠️ ${deleteError}`
            : "هل أنت متأكد من حذف هذه الجهة؟ لا يمكن التراجع عن هذا الإجراء."
        }
