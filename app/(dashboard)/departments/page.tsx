...
export default function DepartmentsPage() {
  ...
  const handleCreate = () => {
    ...
    if (createForm.headUserId) {
      ...
      if (user) {
        ...
        if (otherDepartment) {
          toast.error(
            `هذا المستخدم هو رئيس قسم '${otherDepartment.name}' حالياً. لا يمكن تعيينه كرئيس لقسم آخر.`
          );
          return;
        }
      }
  ...
  const handleUpdate = () => {
    ...
    if (editForm.headUserId) {
      ...
      if (user) {
        ...
        const otherDepartment = departments.find(
          (d) => d.headUserId === user.id && d.id !== selectedDepartment.id
        );
        if (otherDepartment) {
          toast.error(
            `هذا المستخدم هو رئيس قسم '${otherDepartment.name}' حالياً. لا يمكن تعيينه كرئيس لقسم آخر.`
  ...
  const handleToggleActive = (department: DepartmentDto) => {
    if (department.isActive) {
      setConfirmModal({
        ...
        title: "تعطيل القسم",
        message:
          "⚠️ تحذير: تعطيل القسم سيؤدي إلى إزالة جميع الأعضاء ورئيس القسم. هل أنت متأكد؟",
        variant: "warning",
  ...
  const handleRemoveMember = (
    ...
    if (isHead) {
      toast.error(
        "لا يمكن إزالة رئيس القسم من هنا. استخدم زر إزالة الرئيس المخصص."
      );
      return;
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] sm:text-xs text-slate-500">
              إدارة الأقسام وأعضاءها ورؤسائها
            </p>
          </div>
        </div>
              ...
              <table className="w-full text-right text-xs sm:text-sm">
                <thead>
                  <tr className="bg-blue-50 text-slate-700">
                    <th className="p-2 sm:p-3 font-semibold whitespace-nowrap">
                      القسم
                    </th>
                    <th className="p-2 sm:p-3 font-semibold whitespace-nowrap hidden sm:table-cell">
                      الكود
                    </th>
                  ...
                  {paginatedDepartments.map((dept) => (
                    ...
                    >
                      ...
                      <td className="p-2 sm:p-3 whitespace-nowrap">
                        ...
                        >
                          <FontAwesomeIcon
                            icon={dept.isActive ? faCheckCircle : faTimes}
                            className="text-[8px]"
                          />
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 rounded-xl">
                ...
                <div>
                  ...
                  <p className="font-semibold text-slate-800">
                    {viewingDepartment.membersCount}
                  </p>
                </div>
                <div>
