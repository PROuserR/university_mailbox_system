...
import { useDistributionStatus } from "@/hooks/useAnalytics";
...
export default function DistributionStatusReportPage() {
  ...
  const {
    ...
    error,
  } = useDistributionStatus(correspondenceId ? Number(correspondenceId) : null);
  ...
  useEffect(() => {
    if (error) {
      toast.error("فشل تحميل تقرير حالة التوزيع");
    }
  }, [error]);
      ...
      <div className="bg-white rounded-xl border border-blue-100 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {data.receivers.map((receiver, index) => {
            ...
            // ✅ استخدام مفتاح فريد يجمع بين receiverId و index
            const uniqueKey = `${receiver.receiverId}-${index}`;
