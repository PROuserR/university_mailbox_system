...
import {
  useDistributionEditor,
  useDistributeMutation,
} from "@/hooks/useDistribute";
...
export default function DistributionPageClient() {
  ...
  const navigateBack = () => {
    const hasViewCorrespondences =
      delegatedPermissions.includes("ViewCorrespondence");
    if (hasViewCorrespondences) {
  ...
  const currentUsers =
    editorData?.users.filter((u) => u.isSelected === true) ?? [];

  const availableUsers =
    editorData?.users.filter((u) => {
  ...
  const statusEnum = getStatusEnum(editorData?.correspondenceStatus);
  const isDistributeDisabled =
    statusEnum === CorrespondenceStatus.Signed ||
    statusEnum === CorrespondenceStatus.Archived;

  const requireDeanApproval = editorData?.requireDeanApprovalForAll || false;
  const autoApprovePermanent =
    editorData?.autoApprovePermanentReceivers || false;

  const hasChanges =
    JSON.stringify(selectedUsers.sort()) !==
      JSON.stringify(initialSelectedUsers.sort()) || notes !== "";

  // =========================
  // =========================

  const getTotalSelectedUsers = (): number => {
    if (isDean) {
      return selectedUsers.length;
    }

    const permanentCount = permanentUsers.length;
    const selectedCount = selectedUsers.length;
    return permanentCount + selectedCount;
  };

  const hasAnyReceiver = getTotalSelectedUsers() > 0;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!hasAnyReceiver) {
      if (isDean) {
        ...
        toast.error("لا يوجد مستلمون (دائمون أو مختارون)");
      }
      return;
    }

    if (isDean && selectedUsers.length === 0) {
      toast.error("يجب اختيار مستلم واحد على الأقل");
      return;
    }
  ...
  if (!isLoading && !isAuthLoading && editorData && isDistributeDisabled) {
    const statusLabel = getStatusLabel(statusEnum);
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-lg border border-red-100 p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2">
            لا يمكن توزيع المراسلة
          </h2>
          <p className="text-gray-600 mb-6">
            هذه المراسلة{" "}
            <span className="font-semibold text-red-500">{statusLabel}</span>،
            لا يمكن توزيعها في حالتها الحالية.
          </p>
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold transition shadow-md hover:shadow-lg"
          >
  ...
  const statusLabel = getStatusLabel(statusEnum);
  const statusColor = getStatusColor(statusEnum);

  const totalReceivers = getTotalSelectedUsers();
  const permanentCount = permanentUsers.length;
  const selectedCount = selectedUsers.length;
  ...
  return (
    <>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-sm">
              ...
              <div className="flex items-center gap-1.5 text-gray-500">
                ...
                <span className="text-xs">
                  {totalReceivers} مستلم
                  {!isDean && permanentCount > 0 && ` (${permanentCount} دائم)`}
                </span>
              </div>
            </div>
        ...
        <main className="flex-1 overflow-y-auto px-4 md:px-8 py-4 pb-32">
          <div className="max-w-6xl mx-auto space-y-4 px-2 sm:px-3 md:px-4 pb-8">
            ...
            <div className="bg-white rounded-xl border border-yellow-200 shadow-sm p-4">
              ...
              {currentUsers.length === 0 && permanentCount === 0 ? (
      ...
      {/* ===== Confirmation Dialog (مودال مخصص) ===== */}
      {showBackConfirm && (
        <div
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 mb-2">
              ...
              <button
                onClick={() => setShowBackConfirm(false)}
                className="w-7 h-7 rounded-lg hover:bg-gray-100 transition flex items-center justify-center text-gray-400 hover:text-gray-600"
              >
                <FontAwesomeIcon icon={faXmark} className="text-sm" />
              </button>
            </div>
            <p className="text-sm text-gray-500 mb-4">
              هناك تغييرات غير محفوظة. هل أنت متأكد من الرجوع؟ سيتم فقدان
              التغييرات.
            </p>
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 rounded-xl bg-yellow-500 hover:bg-yellow-600 text-white font-medium shadow-sm transition hover:shadow-md text-sm"
              >
                رجوع
              </button>
            </div>
