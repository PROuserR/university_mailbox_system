...
import { MonthlyTrendPointDto } from "@/types/api/analytics.types";
...
export function MonthlyTrendChart({ data }: MonthlyTrendChartProps) {
  ...
  const chartData = data.map((item) => {
    const monthName = MONTHS_AR[item.month] || item.month;
    return {
      ...item,
      name: monthName,
      // ✅ دعم كل من الأسماء الجديدة والقديمة
      incoming: item.distributionsIncoming || item.correspondencesIncoming || 0,
      outgoing: item.distributionsOutgoing || item.correspondencesOutgoing || 0,
      internal: item.distributionsInternal || item.correspondencesInternal || 0,
      total: item.distributions || item.correspondences || 0,
    };
  });
