...
export default function Pagination({
  ...
  return (
    <div className={`p-2 md:p-3 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-2 ${className}`}>
      {/* معلومات الصفحات */}
      {showInfo && (
      ...
      <div className="flex items-center gap-0.5 order-1 sm:order-2 flex-wrap justify-center">
        ...
        >
          <FontAwesomeIcon icon={faAnglesRight} className="h-3 w-3 md:h-4 md:w-4" />
        </button>
        ...
        >
          <FontAwesomeIcon icon={faAnglesLeft} className="h-3 w-3 md:h-4 md:w-4" />
        </button>
      </div>
    </div>
  );
