...
interface AdvancedSearchModalProps {
  ...
  onDistributionStatusChange?: (value: DistributionStatus | undefined) => void;
  correspondenceStatus?: CorrespondenceStatus;
  onCorrespondenceStatusChange?: (value: CorrespondenceStatus | undefined) => void;
  sortField: SortField;
...
// ✅ حالة التوزيع (DistributionStatus) - تبدأ من 1
const distributionStatusOptions: { value: DistributionStatus | ""; label: string }[] = [
  { value: "", label: "الكل" },
  { value: DistributionStatus.Pending, label: "قيد الانتظار" },
  { value: DistributionStatus.Read, label: "مقروء" },
  { value: DistributionStatus.Ignored, label: "متجاهل" },
  { value: DistributionStatus.Revoked, label: "ملغي" },
  { value: DistributionStatus.PendingApproval, label: "بانتظار الموافقة" },
  { value: DistributionStatus.Rejected, label: "مرفوض" },
];

// ✅ حالة المراسلة (CorrespondenceStatus) - تبدأ من 0
const correspondenceStatusOptions: { value: CorrespondenceStatus | ""; label: string }[] = [
  { value: "", label: "الكل" },
...
export function AdvancedSearchModal({
  ...
  // ✅ معالج رقم المراسلة - منع الأرقام السالبة
  const handleNumberChange = (value: string) => {
    if (!onNumberChange) return;
    const num = value ? parseInt(value) : undefined;
    // إذا كان الرقم سالباً أو صفر، نرسل undefined
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl rounded-2xl p-0 shadow-2xl max-h-[90vh] flex flex-col" hideCloseButton>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {/* رقم المراسلة - مع منع الأرقام السالبة */}
          {onNumberChange && (
            <div>
          ...
          {/* ✅ حالة التوزيع (DistributionStatus) */}
          {onDistributionStatusChange && (
            <div>
              <label className="text-xs font-medium text-muted-foreground">حالة التوزيع</label>
              <select
                value={distributionStatus !== undefined ? String(distributionStatus) : ""}
                onChange={(e) => {
                  const value = e.target.value;
                  onDistributionStatusChange(value ? Number(value) as DistributionStatus : undefined);
                }}
                className="w-full mt-1 rounded-lg border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
              >
          ...
          {/* ✅ حالة المراسلة (CorrespondenceStatus) */}
          {onCorrespondenceStatusChange && (
            <div>
              <label className="text-xs font-medium text-muted-foreground">حالة المراسلة</label>
              <select
                value={correspondenceStatus !== undefined ? String(correspondenceStatus) : ""}
                onChange={(e) => {
                  const value = e.target.value;
                  onCorrespondenceStatusChange(value ? Number(value) as CorrespondenceStatus : undefined);
                }}
                className="w-full mt-1 rounded-lg border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
              >
                {correspondenceStatusOptions.map((opt) => (
                  <option key={opt.value} value={opt.value}>
                    {opt.label}
                  </option>
                ))}
