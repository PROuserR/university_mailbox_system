      ...
      <>
        {Children.map(children, (child) => {
          ...
          const newProps = {
            ...
            className: `${(originalProps.className as string) || ''} opacity-50 cursor-not-allowed pointer-events-none`,
            style: { ...(originalProps.style as React.CSSProperties), pointerEvents: 'none' },
          };
...
export const IsAdmin = ({ children, fallback, disableOnUnauthorized = false }: RoleGateProps) => (
  <PermissionGate roles={[UserRole.ADMIN]} fallback={fallback} disableOnUnauthorized={disableOnUnauthorized}>
    {children}
  </PermissionGate>
);

export const IsDean = ({ children, fallback, disableOnUnauthorized = false }: RoleGateProps) => (
  <PermissionGate roles={[UserRole.DEAN]} fallback={fallback} disableOnUnauthorized={disableOnUnauthorized}>
    {children}
  </PermissionGate>
);

export const IsDeanOrAdmin = ({ children, fallback, disableOnUnauthorized = false }: RoleGateProps) => (
  <PermissionGate roles={[UserRole.DEAN, UserRole.ADMIN]} fallback={fallback} disableOnUnauthorized={disableOnUnauthorized}>
    {children}
  </PermissionGate>
);

export const IsEmployee = ({ children, fallback, disableOnUnauthorized = false }: RoleGateProps) => (
  <PermissionGate roles={[UserRole.EMPLOYEE]} fallback={fallback} disableOnUnauthorized={disableOnUnauthorized}>
    {children}
  </PermissionGate>
);

export const IsHeadOfDepartment = ({ children, fallback, disableOnUnauthorized = false }: RoleGateProps) => (
  <PermissionGate roles={[UserRole.HEAD_OF_DEPARTMENT]} fallback={fallback} disableOnUnauthorized={disableOnUnauthorized}>
    {children}
  </PermissionGate>
);

export const IsHeadOfDepartmentOrAdmin = ({ children, fallback, disableOnUnauthorized = false }: RoleGateProps) => (
  <PermissionGate roles={[UserRole.HEAD_OF_DEPARTMENT, UserRole.ADMIN]} fallback={fallback} disableOnUnauthorized={disableOnUnauthorized}>
    {children}
  </PermissionGate>
);
...
interface PermissionGateSimpleProps {
  children: ReactNode;
  fallback?: ReactNode;
  disableOnUnauthorized?: boolean;
}

export const CanManageDelegations = ({ children, fallback, disableOnUnauthorized = false }: PermissionGateSimpleProps) => (
  <PermissionGate
...
export const CanManageCorrespondence = ({ children, fallback, disableOnUnauthorized = false }: PermissionGateSimpleProps) => (
  <PermissionGate
