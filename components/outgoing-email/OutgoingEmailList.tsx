...
import { forwardRef, useMemo, RefObject, useCallback, memo } from "react";
...
const getStatusEnum = (status: any): EmailStatus => {
  if (typeof status === "number") return status as EmailStatus;
  if (typeof status === "string") {
    ...
    return map[status] ?? EmailStatus.Pending;
  }
...
export const OutgoingEmailList = memo(
  forwardRef<HTMLDivElement, OutgoingEmailListProps>(
    (
      {
        items,
        selectedId,
      ...
      const combinedRef = useCallback(
        (node: HTMLDivElement | null) => {
          ...
          if (bottomRef && "current" in bottomRef) bottomRef.current = node;
          if (setBottomRef) setBottomRef(node);
        },
        [ref, bottomRef, setBottomRef]
      );
      ...
      if (isLoading && uniqueItems.length === 0) {
        return (
          <div className="flex h-full flex-col">
            ...
            <div ref={combinedRef} className="h-1" />
          </div>
        );
      }
                  ...
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                        ...
                        {item.messageId && <span>📧 {item.messageId.substring(0, 8)}...</span>}
                        {item.errorMessage && (
