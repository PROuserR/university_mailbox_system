...
export const senderEntityService = {
  ...
  async getAll(): Promise<SenderEntityResponse[]> {
    const response = await myAPI.get<ApiResult<SenderEntityResponse[]>>(
      "/SenderEntity"
    );
  ...
  async create(data: CreateSenderEntityRequest): Promise<SenderEntityResponse> {
    const response = await myAPI.post<ApiResult<SenderEntityResponse>>(
      "/SenderEntity",
      data
    );
    return response.data.data;
  ...
  ): Promise<SenderEntityResponse> {
    const response = await myAPI.put<ApiResult<SenderEntityResponse>>(
      `/SenderEntity/${id}`,
      data
    );
    return response.data.data;
   ...
   */
  async search(searchTerm?: string): Promise<SenderEntityResponse[]> {
