...
function sanitizePendingApprovalData(items: PendingApprovalCorrespondenceDto[]): PendingApprovalCorrespondenceDto[] {
...
export function usePendingApprovals(pageSize: number = 20) {
    const [data, setData] = useState<PagedResponse<PendingApprovalCorrespondenceDto> | null>(null);
    ...
    const loadData = useCallback(async (page: number = 1, isLoadMore: boolean = false) => {
        try {
            const result = await distributionService.getPendingApprovalsGrouped(page, pageSize);
    ...
    const approveSelected = useCallback(async () => {
        ...
        try {
            const count = await distributionService.approveDistributions(selectedItems);
    ...
    const rejectSelected = useCallback(async (reason?: string) => {
        ...
        setIsProcessing(true);
        try {
            const count = await distributionService.rejectDistributions(selectedItems, reason);
    ...
    const approveAllByCorrespondence = useCallback(async (correspondenceId: number) => {
        setIsProcessing(true);
        try {
            const count = await distributionService.approveAllByCorrespondence(correspondenceId);
    ...
    const rejectAllByCorrespondence = useCallback(async (correspondenceId: number, reason?: string) => {
        setIsProcessing(true);
        try {
            const count = await distributionService.rejectAllByCorrespondence(correspondenceId, reason);
    ...
    useEffect(() => {
        ...
        const fetchInitial = async () => {
            try {
                const result = await distributionService.getPendingApprovalsGrouped(1, pageSize);
                if (!isMounted.current) return;
