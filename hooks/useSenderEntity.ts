...
import type { SenderEntityResponse, CreateSenderEntityRequest, UpdateSenderEntityRequest } from "@/types/senderEntity";
...
export function useSenderEntityOperations() {
  ...
  const update = useCallback(async (id: number, data: UpdateSenderEntityRequest) => {
    setIsLoading(true);
    try {
